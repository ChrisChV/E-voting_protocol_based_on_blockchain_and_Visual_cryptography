...
void createTestServer(string user, string ip, string pass ,string leader, string puerto){
    ...
    tempFile<< "\treturn hosts_list"<<endl;
    tempFile<< "hosts_list = getHostList(SERVER_HOSTS_FILE_PATH)"<<endl;
    ...
    tempFile<<"yo = NodoSufAndComis(myHost," + puerto + ", hosts_list, 0, myLeader, toleranceValue, voteUmbral, blockchainPath)"<<endl;
    tempFile.close();
